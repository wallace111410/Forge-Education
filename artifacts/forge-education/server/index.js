...
app.post('/forge-api/child/:childId/rename-agent', (req, res) => {
  ...
  const data = readData();
  const child = data.forge?.children?.[childId] || data.children?.[childId];
  if (!child) return res.status(404).json({ error: 'Child not found' });
...
app.post('/forge-api/session/end', async (req, res) => {
  ...
  child.agentMemory.lastSessionSummary =
    `Last session: ${session.domain} domain, mission ${session.missionId}, ${Math.round((session.duration||0)/60)} minutes.`;
...
STAY WITH THE ANSWER: After a child responds, do not move to the next question. Stay with what they said. Reflect it back. Go one level deeper. Ask a follow-up that comes directly from their words — not from your agenda. Only move forward when something real has been said and genuinely heard. A surface answer is an invitation to go deeper, not a green light to continue. The goal is never to get through the framework. The goal is to find the one thing that actually matters in this conversation today.
