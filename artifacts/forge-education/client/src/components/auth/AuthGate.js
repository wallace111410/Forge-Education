...
const CHILDREN = [
  ...
  },
  {
    ...
    colorLight: '#dcfce7',
    emoji: '⛳',
    greeting: 'Ozzy is waiting'
  }
];

export default function AuthGate({ onChildLogin, onParentLogin }) {
  ...
  React.useEffect(() => {
    if (pin.length === 4 && selected) {
      handleSubmitChildPin();
    }
  }, [pin]);

  React.useEffect(() => {
    if (parentPin.length === 4 && showParentPin) {
      handleSubmitParentPin();
    }
  }, [parentPin]);
    ...
    return (
      <div className="auth-gate parent-auth">
        <div className="auth-header">
          <button className="back-btn" onClick={() => { setShowParentPin(false); setParentPin(''); setError(''); }}>
            ← Back
          </button>
        </div>
        <div className="auth-content">
          ...
          <Keypad
            onDigit={d => setParentPin(prev => prev.length < 4 ? prev + d : prev)}
  ...
  if (selected && child) {
    return (
      <div className="auth-gate child-pin" style={{ '--child-color': child.color, '--child-light': child.colorLight }}>
        <div className="auth-header">
          <button className="back-btn" onClick={() => { setSelected(null); setPin(''); setError(''); }}>
            ← Back
          </button>
        </div>
        <div className="auth-content">
  ...
  return (
    <div className="auth-gate selector">
      ...
      <div className="child-selector">
        {CHILDREN.map(c => (
          <button
            ...
            onClick={() => { setSelected(c.id); setPin(''); setError(''); }}
            style={{ '--card-color': c.color, '--card-light': c.colorLight }}
          >
      ...
      <button
        ...
        onClick={() => { setShowParentPin(true); setError(''); }}
      >
        Parent Admin
      </button>
    </div>
...
function Keypad({ onDigit, onBackspace, loading, accent }) {
  const keys = [
    ['1','2','3'],
    ['4','5','6'],
    ['7','8','9'],
        ...
        <div key={ri} className="keypad-row">
          {row.map((key, ki) => (
            <button
              ...
              onClick={() => {
                if (key === '⌫') onBackspace();
                else if (key !== null) onDigit(key);
              }}
              disabled={loading || key === null}
              style={key && key !== '⌫' ? { '--key-accent': accent } : {}}
            >
              {loading && key === '0' ? '...' : key}
            </button>
          ))}
